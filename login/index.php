...
if(isset($_POST['email']) && isset($_POST['senha']) && !empty($_POST['email']) && !empty($_POST['senha'])){
   ...
   $usuario = $sql->fetch(PDO::FETCH_ASSOC); //FETCH ASSOC PARA RECEBER OS DADOS DE UMA MATRIZ ASSOCIATIVA;
   if($usuario){
    //EXISTE O USUÁRIO;
    ...
    if($usuario['status']=="confirmado"){
           ...
           if($sql = $pdo->prepare("UPDATE usuarios SET token=? WHERE email=? AND senha=?")){
               $sql->execute(array($token,$email,$senha_cript));
               //ARMAZENAR O TOKEN NA SESSÃO (SESSION);
               $_SESSION['TOKEN'] = $token;
               header('location: restrita.php');
    }
    }else{
        $erro_login = "Por favor confirme seu cadastro no seu email cadastrado!";
    }
